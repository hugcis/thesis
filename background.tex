...
\cite{nehanivEvolutionAsynchronousCellular2003}. The authors argue that
...
\cite{garzonRealComputationCellular1993} explores the condition for a
...
of that table quickly exceeds the available memory of most computers.
...
\subsection{Parametrizing and sampling \ac{CA} rules}
...
proposed, having their respective benefits and drawbacks.
...
\begin{figure}[htbp]
  ...
  \caption{The effect of varying $\lambda$ for a 1D binary \ac{CA} rule. A \ac{CA} rule is
    progressively modified along a trajectory of increasing $\lambda$. The rule starts
  with a fixed behavior, becomes periodic and complex when hitting the ``edge of
  chaos''. When $\lambda$ increases further the rule becomes chaotic.}
...
to sample \ac{CA} while scanning a wide range of behavior. The naive method of
uniformly sampling each transition output is not useful in large rule spaces with
multiple states, large neighborhoods or grid dimension higher than 1.
