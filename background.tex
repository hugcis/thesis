...
with the indexes of its neighbors in the grid. In general, a \ac{CA} can be constructed on any
...
function of its local neighborhood at the current time step. The local group of cells is said to be in a particular \emph{configuration}. The function $\boldsymbol{\Phi}$ is applied in
...
looked up from a table or array containing all possible transitions from input to output.
