...
useful to understand the characteristics and applications these two models share
...
complex systems for solving problems, and which we apply in chapter
...
any space $\mathcal{L}$ where such a function can be defined. The space
...
\emph{radius} parameter $r = 1$. The $r=1$ neighborhood is illustrated on Figure
...
weights of a neural network. For example, as long as all the simulation steps
...
With this representation, on can express the transition rule $\Phi$ as a simple
convolutional neural network. This network would be composed of 2 layers which
are shown on Figure~\ref{fig:global_schema}:
...
ones and zeros, that are each the same as a possible configuration of size
...
non-linearity with a vector of 8s as bias, we obtain the input to the second
...
make a reservoir suitable to solve computational task.
...
\begin{itemize}
  \item The inputs need to be mapped to a high-dimensional space, which
        ...
        high-dimensionality ensures the inputs are separated with high
        probability.
...
\ac{RC} has had several successful applications in various fields.
