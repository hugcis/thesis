...
In this chapter, we provide some background on some of the topics we study in
the thesis. We begin by describing the \acf{CA} model, which is the complex
system we focus on in the thesis (Section \ref{sec:cellular-automata-sec}).
...
completely as a special case of \acp{RNN}. From this reformulation, the use of
\acp{CA} in places where \acp{RNN} are successful becomes natural. We describe
the \acf{RC} model (Section \ref{sec:res-models}), which enables harvesting the
computations of complex systems to solve problems and which we apply in chapter
\ref{cha:learn-effic-compl}. This model was developed with \acp{RNN} but we
describe how \acp{CA} can be a suitable replacement and the benefits of this
approach.
...
with its neighbors on the grid. In general, a \ac{CA} can be constructed on
any space $\mathcal{L}$ where this function can be defined. The space
$\mathcal{L}$ specifies an indexing and the relation between the cells. In
practice, regular finite or infinite grids are chosen,
$\mathcal{L} \subset \mathbb{Z}$ or $\mathcal{L} \subset \mathbb{Z}^{2}$. For
example, the grid could be a 1 dimensional torus with 10 cells, that is,
$\mathcal{L}_{{T_{10}}} = \{1, 2, \ldots, 10 \}$. The neighborhood function has
the form
\begin{equation}
  \begin{aligned}
...
The details of an example update step in a 1 dimensional \ac{CA} is shown on
...
associates an output state for each combination of input neighbor states. These
...
compression size of the space-time diagrams of all \ac{ECA} for a fixed-length simulation.
Using a simple k-means clustering technique, he obtained two
groups roughly matching Wolfram’s classes 1 and 2 and classes 3 and 4. We
reproduce these results on figure \ref{subfig:comp_scores}. They offer an
interesting confirmation of Wolfram's classification, which is not qualitative.
...
constraining various parts of the above definition. We list some common ones
here.
...
interesting patterns with particular properties such as the length of a period or the speed
...
to predict and make these types of \ac{CA}
...
range $[0, 1]$. The update function is then a real multivariate function, which we can write
\begin{equation}
  \begin{aligned}
...
This quantity becomes small for large $n$ and rules with an output transition skewed
towards a specific state. For example, the binary game of life rule has 372
transitions that lead to state 0 and 140 that lead to state 1. The
probability of sampling a rule with these transition proportions is
$8.24\mathrm{e}{-26}$, making it very unlikely that a rule of this type is obtained with a
...
of that neural network of a \ac{CA} is entirely determined by the parameters of
that neural network. This allows modifying the \ac{CA} rules like we change the
weights of a neural network. For example, as long as all simulation steps
are differentiable, the backpropagation algorithm can be applied and used to
update a \ac{CA} rule by updating the weights of the neural network with
gradient descent.
...
notation. This parallel is illustrated in Figure~\ref{fig:standard_rnn}.
...
The transformation $\Phi$ operates only on this hidden state. It is equivalent
...
\begin{figure}[htbp]
  ...
  \caption{\label{fig:cell}A \ac{CA} neighborood vector representation example
    with 2 states. A $3\times 3$ square of cells with two states can be
    represented by a $3\times 3 \times 2$ vector. Left: 1D 3-neighbors
    representation. Right: 2D $3\times3$ neighbors representation. The last
    dimension of the vector encodes a ``one-hot'' representation of the state of
    the cell.}
...
are shown in Figure~\ref{fig:global_schema}:

\begin{figure}[htbp]
  \centering
  \includegraphics[width=.9\linewidth]{figures/global_schema}
  \caption{\label{fig:global_schema}A simple representation of any 2-states 2D
    CA rule operating on $3\times 3$ neighborhoods as a 2 layers CNN.}
\end{figure}
...
eight configurations $000, 001, 010, \ldots, 111$. For a radius $r$, this layer
is composed of $k^{2r + 1}$ filters of dimension $k\times (2r + 1)$ with only
ones and zeros, which are each the same as a possible configuration of size
$(2r+1)$ of the neighborhood. The product of each filter with an input
neighborhood will be an integer between $0$ and $9$. By applying a ReLU
nonlinearity with a vector of 8s as bias, we obtain the input to the second
layer, a vector of size $k^{2r + 1}$ for each cell with plays the role of an
indicator of the input configuration.

\paragraph{A second convolutional layer} It has $k^{2r + 1}$ filters of size $1$ that
are $0$ or $1$ depending on the desired output of the transition.
...
outputs could be added to the Game of Life or any other \ac{CA}. We can think of many
possible forms.
...
\item[Vector state] It is simple to add inputs to a \ac{CA} is to
consider the grid as having an additional (possibly read-only) dimension. For
example, a 1D automaton would actually be represented by a vector of dimension
$N\times 2$ where the first vector of size $N$ is the state of the grid and the other
is the input. The update rule would be changed to add this new component
...
read-only memory space. The concatenation is illustrated in Figure~\ref{fig:concat}.
A possible disadvantage of this method is the fact that information needs to
...
However, this becomes more interesting when we consider the input as encoding both
some data and a computer program, as is the case with a Turing machine. We can
then expect a \ac{CA} (or \ac{RNN}) not only to compute the result of a particular
...
Viewing cellular automata as recurrent convolutional neural networks as
...
This is an interesting result, although not very significant since RNN has already
been proven to be
...
been trained to do, it may defeat the purpose of working with a model
capable of spontaneous complex emergent behavior. An open-ended complexity increase
