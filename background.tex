...
Asynchronous \acp{CA} are an appealing model when using large systems for which
...
example, there are $2^9 = 512$ possible $3\times 3$ configurations of a binary Moore neighborhood (see \ref{fig:moore}) in 2D that can each lead to 2 states, which means there exists $2^{512} \approx 10^{154}$ distinct 2D rules with 2 states and a
...
Despite these limitations, several works have studied higher dimensional \acp{CA},
...
Figure ~\ref{fig:standard_rnn} depicts the locations where inputs and 
...
and how they can interact with the \ac{CA}'s rule.
...
\begin{itemize}
  \item \textbf{Vector state.} A straightforward way to add inputs to a \ac{CA} is to
        ...
        dimension. For example, a 1D automaton of size $N$ would actually be represented by
...
\parencite{masonSynapticTransmissionIndividual1991}. The authors observed a interval
...
We define an input sequence to be $(\vx_{t})_{t=1}^{T} \in 
...
A cellular automaton is a discrete system that is not particularly designed to
...
concatenation is done over this dimension. In higher-dimensional spaces,
