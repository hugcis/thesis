...
share (Section \ref{sec:cell-autom-rnns}). \acp{CA} can be completely reformulated as a special case of \acp{RNN}. With this reformulation, using
...
\begin{description}
  ...
  \item[Class 2] All initial configurations relax after a transient period to some
        ...
        finite lattices always have a periodic behavior because there
...
a binary representation. Example simulations of the \ac{ECA} rule 150 starting 
from various initial states are shown on Figure~\ref{fig:eca_150}. In these 
representations, time flows from top to bottom, and each row of cells represents 
a single CA internal state at a single time step.
...
only to date, although rule 54 is postulated to be as well) Turing complete \ac{ECA}.
...
asynchronicity is a strong advantage in constructing systems that can evolve
within \acp{CA}.
...
sample a significant portion of the rule space. A good parametrization allows one to scan a wide range of behavior of \ac{CA} by traversing the space in an
interesting way.
...
transitions to 0). It is therefore very unlikely that a rule of the type of game
...
that it preferably samples a tuple with one of their values dominating the others.
...
We write the definition of a \ac{CA} with a formalism and notation inspired by the \ac{RNN}. This parallel is illustrated in figures \ref{fig:ca_cnn} and
...
zeros expected for a one in the $k$-th position. A neighborhood of size $3$ in a
...
Figure~\ref{fig:standard_rnn} shows clearly where inputs and outputs could
