...
sequences, the ``algorithmic complexity'', may also be called a measure of
...
In this section, we list several ``complexity'' measures that have influenced
our research.
...
- Other examples of emergence include (i) the characteristic  [tyson76], (ii)  [hass88a], and,
more recently, (iii) A of figure which depicts
\begin{itemize}
...
very large. \ac{CA} rules are a good example of such search space, and genetic
algorithms were successfully used to evolve rules to perform complex
computations \parencite{mitchellEvolvingCellularAutomata1996}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=.8\linewidth]{figures/evolutionary_diagram.pdf}
  \caption{Illustration of the general principle common to many evolutionary
    algorithms. There are three virtual spaces: ($G$) the genotype space where
    the candidate solutions are generated through some encoding. ($P$) the
    phenotype space, which corresponds to the decoded solution from a genotype
    $x$. It can be the policy of an agent, the rule of a \ac{CA}, etc. ($B$) the
    behavior space, which is the space from which candidate solutions are
    evaluated. The small subspace $\mathcal{G}$ is the goal space, corresponding
  to the set of behaviors that would be considered successful.}
...
bits. Every generation, a population of solutions is evaluated. The best
solutions are kept and combined to form the candidates for the next generation.
Random mutations may also be applied to introduce randomness in the search.
...
\begin{figure}[htbp]
  ...
  the density is lower, it goes to a state with white cells.}
