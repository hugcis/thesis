...
The aim of this thesis is to develop methods that study and use 
computations that take place in complex dynamical systems to create 
...
\end{figure}
  ...
  \item Measure the fraction of systems that have the most complex and rapidly
        ...
        different methods to measure evolving complexity and understand when
...
\end{figure}
  ...
  \item Apply promising systems to challenging tasks where classical machine
        ...
        standard learning tasks and to figure out if it improves the performance or
...
structure from untagged data. In machine learning, supervision refers to a
label, a number, or a collection of numbers that represents the expected outcome
of some input data. Our goal is to construct models that develop autonomously,
so we first seek systems that behave this way and postpone the issue of
learning to optimize a particular objective function. We are particularly
interested in systems that can develop on their own through internal evolution
rules, without any input.

In this thesis, we work toward achieving these goals, focusing on one particular
complex system: the \acl{CA}. This model has been extensively studied due to
its simple definition and surprisingly complex behavior. We provide a
detailed description of \aclp{CA} in Section \ref{sec:cellular-automata-sec}.


\section{Motivation}

Complex dynamical systems could overcome the problems with existing learning
algorithms, such as difficulties in generalization, robustness, or the ability to learn
continuously. The natural intelligence of biological systems seems to rely on
emerging properties selected through evolution. For example, biological life
exhibits a pattern of major evolutionary transitions where lower-level
autonomously replicating entities merge to form a single more complex corporate
body \parencite{lorenzEmergenceModularityBiological2011}. This is believed to
have occurred for the emergence of eukaryotic cells, as well as for multicellular
...
So far, it is unclear what algorithmic properties could lead a process to
display a similar trajectory in its state space. Producing emerging phenomena
similar to those of nature \emph{in silico} is challenging. There is a long history
of algorithms that attempt to apply evolutionary design to solve particular
tasks, called evolutionary algorithms
...
However, many of these methods focus on searching the space of solutions through
several variation mechanisms, such as mutations and crossovers. Although effective
in solving precise tasks, these methods obscure another crucial component of
natural evolution by using an explicit fitness function.
...
regression that maps that internal state to desired outputs (see 
Section \ref{sec:res-models}).
